...
export const musicExecutePlay = async (msg: Message, musicEntity: musicEntity, resource: AudioResource) => {
  //기본 함수
  const subscription = musicEntity.subscription;
  const audioPlayer = musicEntity.subscription.player;
  ...
  audioPlayer.on('error', error => {
    musicEntity.textChannel.send(`에러났어요 ㅠㅠ (${error.message})\n> 에러가 난 곡 이름: ${error.resource.metadata.title}`);
    console.log(error);
    audioPlayer.stop();
  });
  ...
  audioPlayer.once(AudioPlayerStatus.Idle, async (player) => {
    ...
    if (option.loop && !option.skip) {
      const meta = audioPlayer.resource.metadata;
      ...
      nextSong.volume.setVolume(0.5 / option.volumeMagnification * Number(!option.mute));//노래 사운드 최초 설정해주는 곳
      musicEntity.songs.push(nextSong);
    }

    if (musicEntity.songs.length) {
      //다음 노래 있으면 틀어주는 코드
      const nextSong = musicEntity.songs.shift();
      nextSong.volume.setVolume(player.resource.volume.volume * Number(!option.mute));
  ...
  collector.on('collect', async i => {
    ...
    switch (i.customId) {
      ...
      case "🔉":
        ...
        resource.volume.setVolume(Math.max(resource.volume.volume - 1 / (10 * volumeMagnification), 0));
      ...
      case "🔊":
        ...
        if (subscription.option.mute) return msgSungok.channel.send("음소거 중이에요.");
        resource.volume.setVolume(Math.min(resource.volume.volume + 1 / (10 * volumeMagnification), 1 / volumeMagnification));
        msgSungok.channel.send(`현재 볼륨:${Math.round(resource.volume.volume * 100 * volumeMagnification)}%`);
        break;
      ...
      case "🔇":
        ...
        if (subscription.option.mute) {//뮤트 걸리고 나서
          ...
          resource.volume.setVolume(subscription.option.volume);
