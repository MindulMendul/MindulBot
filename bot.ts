...
bot.on('messageCreate', async (msg) => {
	...
	if (!getCmd) {//명령어 인식 못하는 거 거름
		channel.send("명령어를 인식하지 못했어요 ㅠㅠ 명령어를 다시 한 번 확인해주세요!"); return;
	}
	if (guildCmdQueue.get(`${guild.id}${getCmd.type}`) == undefined)//길드 명령어 큐 만들기
	...
	try {
		if (checkGuildCmdQueue.length == 0) { //아무것도 실행 안 되어 있으면 실행
			...
			if (checkPermissions(msg, getCmd.permission))
				await getCmd.execute(msg, args); //실행이 끝날 때까지 대기
		...
		channel.send(`${command} 명령어 입력에 문제가 생겼어요! 우리 주인님이 고생할 거라 생각하니 기분이 좋네요 ㅎㅎ\n${error}`);
